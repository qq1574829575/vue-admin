      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          request({
            ...
            method: 'post',
            baseURL: 'http://www.unifiedplatform.guolianrobot.com',
            data: 'param=' + encodeURIComponent(rsaUtil.encryption2(param)) + '&teacherNoArr=' + JSON.stringify(teacherNoArr) + '&fileId=' + this.gongwen.fileId + '&remark=' + this.form.remark + '&superId=' + this.gongwen.recordId + '&adminFlag=' + this.adminFlag
          }).then(res => {
            if (res.code === 1) {
              this.$notify({
                title: '提示',
